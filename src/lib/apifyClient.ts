...
async function executeWithToken(videoId: string, apiToken: string, accountInfo: ApifyAccountInfo): Promise<{ transcript: string, metadata: any }> {
    ...
    // Se o Apify retornar status 'error' (ex: limite atingido), lançamos erro
    // para que o loop de failover (no fetchYoutubeTranscriptFromApify) possa tentar a próxima chave.
    if (result.status === 'error' || (result.message && typeof result.message === 'string' && result.message.includes('limit'))) {
        console.warn(`[Apify] ⚠️ Erro retornado pela API na conta ${acct}: ${result.message}`);
        throw new Error(result.message || 'Erro desconhecido na execução do Actor');
    }
    ...
    for (const key of ['transcript', 'transcript_text', 'text', 'transcript_segments', 'title', 'video_id', 'status', 'message']) {
    ...
    if (transcript) {
        ...
        return {
            transcript,
            metadata: { ...result, _apifyAccount: accountInfo.username, _source: source }
        };
    }
